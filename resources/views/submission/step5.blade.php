...
<div class="min-h-screen bg-[#0f1115] text-white font-['Outfit'] py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-4xl mx-auto">
                ...
                @foreach(['Submission Type', 'Service Level', 'Card Details', 'Shipping', 'Review', 'Payment'] as $index => $step)
                    <div class="flex flex-col items-center">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm mb-2 transition-all duration-300 relative
                            {{ $index + 1 <= 5 ? 'bg-gradient-to-r from-blue-500 to-purple-600 shadow-[0_0_15px_rgba(59,130,246,0.5)] scale-110' : 'bg-[#1f2937] text-gray-500' }}">
                            @if($index + 1 < 5)
        ...
        <div class="bg-[#1a1d24]/80 backdrop-blur-xl rounded-2xl border border-white/5 p-8 shadow-2xl relative overflow-hidden group">
            ...
            <div class="mb-8">
                <h2 class="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-400 mb-2">Order Review</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                <!-- Submission Info -->
                <div class="bg-[#0f1115] rounded-xl p-6 border border-white/10">
            ...
            <div class="mb-8 overflow-hidden rounded-xl border border-white/10">
                <table class="w-full text-left text-sm text-gray-400">
                    <thead class="bg-[#1f2937] text-white uppercase font-medium">
                        <tr>
                    ...
                    <tbody class="divide-y divide-white/5 bg-[#0f1115]">
                            ...
                            @foreach($submission->cards as $card)
                                ...
                                <tr class="hover:bg-white/5 transition-colors">
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="text-xs text-gray-500">{{ $card->set_name }} {{ $card->year ? '('.$card->year.')' : '' }} #{{ $card->card_number }}</div>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        @if($card->labelType)
                                            <div class="flex flex-col items-center">
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100/10 text-blue-400 border border-blue-500/20">
                                                    {{ $card->labelType->name }} 
                                                </span>
                    ...
                    <tfoot class="bg-[#1f2937]">
                        <tr>
                            <td colspan="3" class="px-6 py-4 text-right font-bold text-white">Total:</td>
                            <td class="px-6 py-4 text-right font-bold text-blue-400 text-lg">€{{ number_format($totalCost, 2) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
            ...
            <div class="flex justify-between items-center pt-6 border-t border-white/10">
                <a href="{{ route('submission.step4') }}" class="px-6 py-3 rounded-xl bg-[#1f2937] text-gray-300 hover:text-white hover:bg-[#2d3748] transition-all duration-300 font-medium border border-white/5">
                    Back to Shipping
                </a>
                
                <a href="{{ route('submission.step6') }}" class="group relative px-8 py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-green-600 text-white font-bold shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:shadow-[0_0_30px_rgba(16,185,129,0.5)] transition-all duration-300 hover:scale-[1.02]">
