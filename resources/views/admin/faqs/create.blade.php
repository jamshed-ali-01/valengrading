    ...
    <div class="bg-[#232528]/80 backdrop-blur-xl border border-white/5 rounded-2xl p-8 shadow-2xl">
        <form method="POST" action="{{ route('admin.faqs.store') }}" class="space-y-6">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex flex-col gap-2">
                                <label class="flex items-center gap-3 p-3 rounded-lg border border-white/5 bg-[#1C1E21] hover:bg-[#2A2C30] cursor-pointer transition-all">
                                    <input type="radio" name="show_on_home" value="0" {{ old('show_on_home', 0) == 0 ? 'checked' : '' }} class="w-4 h-4 text-red-500 bg-gray-700 border-gray-600 focus:ring-red-500 focus:ring-2">
                                ...
                                <label class="flex items-center gap-3 p-3 rounded-lg border border-white/5 bg-[#1C1E21] hover:bg-[#2A2C30] cursor-pointer transition-all">
                                    <input type="radio" name="show_on_home" value="1" {{ old('show_on_home', 0) == 1 ? 'checked' : '' }} class="w-4 h-4 text-red-500 bg-gray-700 border-gray-600 focus:ring-red-500 focus:ring-2">
                                    <div class="flex flex-col">
            ...
            <div class="pt-4 flex items-center justify-end gap-4">
                <a href="{{ route('admin.faqs.index') }}" class="px-6 py-3 rounded-xl bg-white/5 text-gray-300 hover:text-white hover:bg-white/10 transition-all font-medium">Cancel</a>
