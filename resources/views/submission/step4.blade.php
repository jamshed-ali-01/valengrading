...
<head>
    ...
    <title>Shipping Information - ValenGrading</title>
...
<div class="min-h-screen bg-[#0f1115] text-white font-['Outfit'] py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-3xl mx-auto">
                ...
                @foreach(['Submission Type', 'Service Level', 'Card Details', 'Shipping', 'Review', 'Payment'] as $index => $step)
                    <div class="flex flex-col items-center">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm mb-2 transition-all duration-300 relative
                            {{ $index + 1 <= 4 ? 'bg-gradient-to-r from-blue-500 to-purple-600 shadow-[0_0_15px_rgba(59,130,246,0.5)] scale-110' : 'bg-[#1f2937] text-gray-500' }}">
                            @if($index + 1 < 4)
        ...
        <div class="bg-[#1a1d24]/80 backdrop-blur-xl rounded-2xl border border-white/5 p-8 shadow-2xl relative overflow-hidden group">
            ...
            <div class="mb-8">
                <h2 class="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-400 mb-2">Shipping Information</h2>
            ...
            <form action="{{ route('submission.storeStep4') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <input type="text" name="full_name" id="full_name" required value="{{ old('full_name', $shippingAddress->full_name ?? '') }}" 
                            class="w-full bg-[#0f1115] border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all placeholder-gray-600">
                    </div>
                ...
                <div class="flex justify-between items-center pt-6 border-t border-white/10">
                    <a href="{{ route('submission.step3') }}" class="px-6 py-3 rounded-xl bg-[#1f2937] text-gray-300 hover:text-white hover:bg-[#2d3748] transition-all duration-300 font-medium border border-white/5">
                        Back to Cards
                    </a>
