    ...
    <div class="max-w-3xl mx-auto">
                ...
                @foreach(['Submission Type', 'Service Level', 'Card Details', 'Shipping', 'Review', 'Payment'] as $index => $step)
                    <div class="flex flex-col items-center">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm mb-2 transition-all duration-300 relative
                            {{ $index + 1 <= 1 ? 'bg-gradient-to-r from-blue-500 to-purple-600 shadow-[0_0_15px_rgba(59,130,246,0.5)] scale-110' : 'bg-[#1f2937] text-gray-500' }}">
                            @if($index + 1 < 1)
        ...
        <div class="bg-[#1a1d24]/80 backdrop-blur-xl rounded-2xl border border-white/5 p-8 shadow-2xl relative overflow-hidden group">
            ...
            <div class="mb-8 text-center">
                <h2 class="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-400 mb-2">Start Your Submission</h2>
            ...
            <form action="{{ route('submission.storeStep1') }}" method="POST">
                ...
                <div class="mb-8">
                    ...
                    <input type="text" id="submission_name" name="submission_name" 
                        ...
                        class="w-full bg-[#0f1115] border border-white/10 text-white text-lg rounded-xl focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 block p-4 transition-all placeholder-gray-600" 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach($types as $type)
                            <div class="relative">
                                ...
                                <label for="type_{{ $type->id }}" class="block text-left p-5 bg-[#0f1115] border border-white/10 rounded-xl cursor-pointer peer-checked:border-blue-500 peer-checked:bg-white/5 transition-all hover:bg-white/5 shadow-md group">
                                    <div class="flex items-center justify-between mb-1">
                                        <span class="font-bold text-lg text-white group-hover:text-blue-400 transition-colors">{{ $type->title }}</span>
                                        <div class="w-5 h-5 rounded-full border-2 border-gray-600 peer-checked:bg-blue-500 peer-checked:border-blue-500"></div>
                                    </div>
                ...
                <div class="flex justify-end pt-6 border-t border-white/10">
                    <button type="submit" class="w-full md:w-auto text-white bg-gradient-to-r from-blue-500 to-purple-600 font-bold rounded-xl text-lg px-8 py-4 shadow-[0_0_20px_rgba(59,130,246,0.3)] hover:shadow-[0_0_30px_rgba(59,130,246,0.5)] transform transition hover:scale-[1.02]">
